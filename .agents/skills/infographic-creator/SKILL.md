...
description: Create or insert AntV Infographic visuals for astro-koharu content. Use for complete infographic authoring or blog integration; use infographic-syntax-creator when the user wants only DSL output.
---
...
## astro-koharu project mode

In this repository, default to an `infographic` fenced code block that the existing Markdown renderer consumes. Insert it into the requested post, or return the block when no target file was requested. Create a standalone HTML renderer only when the user explicitly asks for one. Use the version pinned by this project's `package.json`; never introduce an `@latest` runtime into project content.
...
For astro-koharu posts, place the final syntax in a fenced `infographic` code block and rely on the project's existing renderer. If the user explicitly requests a standalone HTML file, generate one with the following steps:
...
1. Create a complete HTML file with the following structure:
   ...
   - Resolve the project's pinned AntV Infographic version and include that exact version rather than `@latest`
...
2. Create the HTML file named `<title>-infographic.html` with the available project file-editing tool
