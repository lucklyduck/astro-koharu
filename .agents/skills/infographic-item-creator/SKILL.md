...
description: Generate or update AntV Infographic Item source when the selected workspace contains src/designs/items. Do not use for astro-koharu's package-consumer integration or for editing node_modules.
---
...
The astro-koharu repository consumes the published `@antv/infographic` package and does not own `src/designs/items`. Do not create that missing source tree or edit `node_modules`. Continue only when the user supplies a compatible AntV Infographic source checkout.
